...
Use for schedule questions. The daily morning briefing orchestrates `calendar_fetch.py` per linked account (see the **morning-brief skill**). See the **calendar skill** for tool usage.
...
- Morning briefing task is due AND the brief skill's gather succeeds (not SKIP)
<!-- CAPABILITY: finance_plaid -->
